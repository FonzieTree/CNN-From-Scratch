...
for i in range(10):
    ...
    kernel3 = np.maximum(epsilon, kernel3)
    kernel2 = np.maximum(epsilon, kernel2)
    kernel1 = np.maximum(epsilon, kernel1)
        ...
        for k in range(256):
            ...
            for o in range(2):
                for p in range(2):
                    ...
                    dkernel3 += dconv3[j, k, o, p]/relu2[j, :, o:o+3, p:p+3]
        dkernel3 = dkernel3/256
        # layer2
        drelu2[np.argwhere(pool2[j, :, :, :]<=0)] = 0
        dpool2 = drelu2
        for k in range(128):
            for o in range(5):
                for p in range(5):
            ...
            for o in range(11):
                for p in range(11):
                    ...
                    dkernel2 += dconv2[j, k, o, p]/relu1[j, :, o:o+3, p:p+3]
        dkernel2 = dkernel2/128
	# layer1
        drelu1[np.argwhere(pool1[j, :, :, :]<=0)] = 0
        dpool1 = drelu1
        for k in range(64):
             for o in range(13):
                 for p in range(13):
             ...
             for o in range(26):
                 for p in range(26):
                    dkernel1 += dconv1[j, k, o, p]/x[j, o:o+3, p:p+3]
    dkernel1 = dkernel1/64
